...
import { Show, Announcement } from '../types';
import { cn, formatTime12 } from '../lib/utils';
import { AnnouncementConflict, ShowConflict } from './CalendarTab';
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden border border-slate-200">
        ...
        <div className="p-5 space-y-4">
          <p className="text-xs text-slate-650 font-bold leading-relaxed">
            This will permanently remove the announcement. If you want to keep it, but suspend it, cancel the delete and instead choose "suspend".
          </p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden border border-slate-200">
        ...
        <div className="p-5 space-y-3 text-xs font-bold text-slate-600">
          <p className="leading-relaxed">
            You have unsaved changes in this announcement window.
          </p>
...
interface AnnouncementConflictModalProps {
  pendingSaveAnnouncement: { updatedList: Announcement[]; conflicts: AnnouncementConflict[] } | null;
  isSaving: boolean;
  onContinueEditing: () => void;
  onSaveAnyway: () => void;
}

export function AnnouncementConflictModal({
  pendingSaveAnnouncement,
  isSaving,
  onContinueEditing,
  onSaveAnyway,
}: AnnouncementConflictModalProps) {
  if (!pendingSaveAnnouncement) return null;
    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-[110] animate-in fade-in duration-150">
      <div className="bg-white rounded-xl shadow-2xl max-w-md w-full p-5 border border-slate-200 space-y-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="text-sm font-bold text-slate-900">Announcement Conflict Detected</h3>
        ...
        <div className="p-3 bg-amber-50/80 border border-amber-200 rounded-lg text-xs italic text-amber-900 space-y-1.5 max-h-40 overflow-y-auto">
          {pendingSaveAnnouncement.conflicts.map((c, idx) => (
        ...
        <p className="text-xs text-slate-600 font-medium">
          Are you sure you want to save this announcement with the conflict?
        </p>
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3 bg-slate-50 rounded-lg p-3 text-xs">
            ...
            <div className="col-span-2 border-t border-slate-200/50 pt-2">
              <span className="text-slate-400 uppercase font-bold block mb-0.5 text-xs">Announcement Timing</span>
              <span className="font-bold text-slate-755 block font-mono">
                {show.day} at {formatTime12(`${show.startHour.toString().padStart(2, '0')}:${(show.startMinute || 0).toString().padStart(2, '0')}`)}
              </span>
            </div>
            <div className="col-span-2 border-t border-slate-200/50 pt-2">
