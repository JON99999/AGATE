import { Announcement, Show } from '../types';
import {
...
function formatIsoDateTime12(isoStr: string): string {
  if (!isoStr) return '';
  const [datePart, timePart] = isoStr.split('T');
  if (!timePart) return datePart;
  return `${datePart} ${formatTime12(timePart)}`;
}
...
export function getAnnouncementGaps(announcementsList: Announcement[], now: Date = new Date()): AnnouncementGap[] {
    ...
    if (timeGated.length > 0) {
      ...
      if (sorted.length > 0 && sorted[0].startDate) {
        ...
        if (firstStart && firstStart > nowIso) {
          const formattedDate = formatIsoDateTime12(firstStart);
          gaps.push({
            announcementId: s.id,
      ...
      for (let i = 0; i < sorted.length - 1; i++) {
        ...
        if (aEnd && bStart && aEnd < bStart && !isContiguousMidnightTransition(aEnd, bStart)) {
          const startFormatted = formatIsoDateTime12(aEnd);
          const endFormatted = formatIsoDateTime12(bStart);
          gaps.push({
            announcementId: s.id,
            announcementName: s.name,
            type: 'coverage_gap',
      ...
      if (sorted.length > 0) {
        ...
        if (lastItem.endDate && lastItem.endDate.trim()) {
          ...
          if (!parentEnd || (parentEnd > lastEnd && !isContiguousMidnightTransition(lastEnd, parentEnd))) {
            const endFormatted = formatIsoDateTime12(lastEnd);
            gaps.push({
              announcementId: s.id,
              announcementName: s.name,
              type: 'trailing_gap',
...
export function getShowGaps(showsList: Show[]): ShowGap[] {
  ...
  const formatWeekMin = (min: number) => {
    ...
    const h = Math.floor((norm % 1440) / 60);
    const m = (norm % 60).toString().padStart(2, '0');
    const timeStr = formatTime12(`${h.toString().padStart(2, '0')}:${m}`);
    return { dayName: daysOrderList[dIdx], timeStr, dIdx };
  };
