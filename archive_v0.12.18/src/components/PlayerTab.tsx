...
export default function PlayerTab({ 
  ...
  useEffect(() => {
    const handleLogged = (eventOrEntry?: any) => {
        ...
        if (logEntry && logEntry.status === 'backup play' && prev) {
          if (prev.interstitialId && prev.slotKey && prev.slotISO) {
            ...
            if (targetInterstitial) {
              ...
              playBackupAudioTrack(logEntry.mp3Name || prev.backupMp3Url, prev.slotKey, targetInterstitial, slotDate, true);
            }
          }
        }
        return null;
      ...
      if ((playMode === 'Playlist' || playMode === 'Prerecord') && activeShow && logEntry?.interstitialId) {
        ...
        const activeMp3 = targetInterstitial ? getActiveMp3ForSlot(targetInterstitial, new Date(logEntry.interstitialTime || logEntry.timestamp || Date.now())) : null;
        ...
        const updatedPlayed = {
          ...
          [logEntry.interstitialId]: {
            ...
            fileName: logEntry.mp3Name || activeMp3?.mp3Url || (logEntry.status === 'backup play' ? `${targetInterstitial?.name || 'Interstitial'} (backup mp3)` : 'Live Read'),
  ...
  }, [interstitials, effectiveShow, playlistShow, playMode]);
  ...
  }, [interstitials, nowClock]);

  const activeMp3Urls = useMemo(() => {
    const urls: string[] = [];
    interstitials.forEach(s => {
      if (!s.enabled) return;
      const activeMp3 = getActiveMp3ForSlot(s, nowClock);
      if (activeMp3?.mp3Url) {
  ...
  }, [interstitials, nowClock, playMode, playlistTracks]);
        ...
        >
          ...
          >
            ...
            <div className="space-y-1">
              ...
              {activeLiveReadOverlay.name || activeLiveReadOverlay.interstitialName ? (
        ...
        <LiveReadPopout
          ...
          onLogCommit={(logEntry) => {
            if (playMode !== 'Export' && activeLiveReadOverlay.playMode !== 'Export' && !activeLiveReadOverlay.isPreview) {
              onLog(logEntry);
              const targetDate = new Date(logEntry.interstitialTime || logEntry.timestamp || Date.now());
              const activeShow = getShowForSlot(targetDate);
              if ((playMode === 'Playlist' || playMode === 'Prerecord') && activeShow && logEntry.interstitialId) {
                const targetInterstitial = interstitials.find(s => s.id === logEntry.interstitialId);
                const activeMp3 = targetInterstitial ? getActiveMp3ForSlot(targetInterstitial, targetDate) : null;
                const durSec = activeMp3?.duration ? parseInt(activeMp3.duration, 10) || 60 : 60;
                const mins = Math.floor(durSec / 60);
                const secs = durSec % 60;
                const durFmt = `${mins}:${secs.toString().padStart(2, '0')}`;
          ...
          onPlayBackupMp3={(backupUrl) => {
            ...
            if (activeLiveReadOverlay.interstitialId && activeLiveReadOverlay.slotKey && activeLiveReadOverlay.slotISO) {
              ...
              if (targetInterstitial) {
                ...
                playBackupAudioTrack(backupUrl, activeLiveReadOverlay.slotKey, targetInterstitial, slotDate, true);
              }
            }
          }}
        />
  ...
  const playlistText = useMemo(() => {
    ...
    itemsToExport.forEach((item, idx) => {
      ...
      if (ext === '.mp3') {
        ...
        m3uLines.push(`#EXTINF:-1,${paddedIdx} ${typePrefix} - ${item.slotTime} - ${item.interstitialName}`);
                                    ...
                                    {isCurrentlyPlaying ? (
                                      ...
                                      <>
                                        ...
                                        <span className="text-xs font-bold text-green-700 uppercase tracking-tighter">
                                          {isScript ? (playedLog?.status === 'backup play' ? 'Played' : 'Read') : 'Played'} {playedLog ? format(parseISO(playedLog.logTimeStamp || playedLog.timestamp), 'HH:mm') : ''}
                                        </span>
                                      </>
                                    ) : (isMissedRecent || isMissedOld) ? (
                                      <>
                                        ...
                                        {exported ? (
                                          ...
                                          <>
                                            ...
                                            <span className="text-xs font-bold text-green-600 uppercase tracking-tighter">
                                              {isScript ? (playedLog?.status === 'backup play' ? 'Played' : 'Read') : 'Played'} {playedLog ? format(parseISO(playedLog.logTimeStamp || playedLog.timestamp), 'HH:mm') : format(new Date(), 'HH:mm')}
                                            </span>
                                          </>
                                        ) : isMissedRecent || isMissedOld ? (
                                          <>
