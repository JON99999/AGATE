...
interface LiveReadPopoutProps {
  ...
  initialAnnouncementName?: string;
  initialAnnouncementTime?: string;
  initialLoggedTime?: string;
  initialShowId?: string;
  initialShowName?: string;
  initialHostName?: string;
  initialShowDateTime?: string;
  backupMp3Url?: string;
  playMode?: 'Live' | 'Prerecord' | 'Playlist' | 'Export';
  onClose?: () => void;
...
export default function LiveReadPopout({
  ...
  initialAnnouncementName = '',
  initialAnnouncementTime = '',
  initialLoggedTime = '',
  initialShowId = '',
  initialShowName = '',
  initialHostName = '',
  ...
  const [announcementName, setAnnouncementName] = useState(initialAnnouncementName);
  const [announcementTime, setAnnouncementTime] = useState(initialAnnouncementTime);
  ...
  const [showDateTime, setShowDateTime] = useState(initialShowDateTime);
  ...
  useEffect(() => {
    try {
      const savedTheme = (localStorage.getItem('amp_theme') as ThemeId) || 'light';
      applyTheme(savedTheme);
    } catch (e) {}

    const handleStorage = (e: StorageEvent) => {
      if (e.key === 'amp_theme' && e.newValue) {
  ...
  useEffect(() => {
    if (initialShowId !== undefined) setShowId(initialShowId);
    if (initialShowName !== undefined) setShowName(initialShowName);
    if (initialHostName !== undefined) setHostName(initialHostName);
    if (initialShowDateTime !== undefined) setShowDateTime(initialShowDateTime);
  }, [initialShowId, initialShowName, initialHostName, initialShowDateTime]);
  ...
  const handlePlayBackupMp3Action = async () => {
    ...
    const logEntry: LogEntry = {
      ...
      announcementTime: announcementTime || commitTimestamp,
      mp3Name: backupMp3Url,
      announcementName: announcementName || fileName || 'Live Read (Backup MP3)',
      announcementId: announcementId,
      status: 'backup play',
      playMode: (playMode as any) || 'Live',
  ...
  useEffect(() => {
    async function loadInitialData() {
      ...
      if (!isOverlay && (window as any).electronAPI && (window as any).electronAPI.getLiveReadData) {
        try {
          ...
          if (ipcData) {
            ...
            setAnnouncementId(ipcData.announcementId || ipcData.scheduleId || '');
            setAnnouncementName(ipcData.announcementName || ipcData.scheduleName || '');
            setAnnouncementTime(ipcData.announcementTime || ipcData.scheduledTime || '');
            setLoggedTime(ipcData.initialLoggedTime || '');
            setBackupMp3Url(ipcData.backupMp3Url || '');
            if (ipcData.showId) setShowId(ipcData.showId);
            if (ipcData.showName) setShowName(ipcData.showName);
            if (ipcData.hostName) setHostName(ipcData.hostName);
            if (ipcData.showDateTime) setShowDateTime(ipcData.showDateTime);
            if (ipcData.playMode) {
              setPlayMode(ipcData.playMode);
            }
      ...
      const sId = params.get('announcementId') || params.get('scheduleId') || '';
      const sName = params.get('announcementName') || params.get('scheduleName') || '';
      const sTime = params.get('announcementTime') || params.get('scheduledTime') || '';
      ...
      const qShowId = params.get('showId') || '';
      const qShowName = params.get('showName') || '';
      const qHostName = params.get('hostName') || '';
      const qShowDateTime = params.get('showDateTime') || '';

      if (qShowId) setShowId(qShowId);
      if (qShowName) setShowName(qShowName);
      if (qHostName) setHostName(qHostName);
      if (qShowDateTime) setShowDateTime(qShowDateTime);

      if (pMode && (pMode === 'Live' || pMode === 'Prerecord' || pMode === 'Playlist' || pMode === 'Export')) {
        setPlayMode(pMode as any);
      }
      ...
      if (f) {
        setFileName(f);
        setAnnouncementId(sId);
        setAnnouncementName(sName);
        setAnnouncementTime(sTime);
  ...
  const handleLogAsRead = async () => {
    ...
    const logEntry: LogEntry = {
      ...
      announcementTime: announcementTime || new Date().toISOString(),
      mp3Name: fileName || 'Unknown Script',
      announcementName: announcementName || 'Live Read Announcement',
      announcementId: announcementId || 'manual',
      status: 'played',
      playMode: (playMode as any) || 'Live',
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <h1 className="text-sm font-black uppercase tracking-wider text-slate-200 truncate leading-none">
              {announcementName || 'Live Read Reader'}
            </h1>
