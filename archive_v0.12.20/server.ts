...
const DATA_DIR = IS_PORTABLE ? path.join(BASE_DIR, 'data') : path.join(BASE_DIR, 'data');
...
// For portable instances, interstitial-er_settings.json sits in the launch folder (BASE_DIR)
// For installed/dev instances, it sits in DATA_DIR.
const SETTINGS_FILE = IS_PORTABLE
  ? path.join(BASE_DIR, 'interstitial-er_settings.json')
  ...
  app.post('/api/browse-folder', (req, res) => {
    try {
      if (electronDialog) {
        ...
        if (IS_PORTABLE && BASE_DIR && fs.existsSync(BASE_DIR)) {
