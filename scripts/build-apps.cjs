...
const pkgBakPath = path.join(rootDir, 'package.json.bak');
...
async function syncRemoteIcons() {
  ...
  for (const mode of modes) {
    const modeDir = path.join(rootDir, 'src', 'assets', 'images', mode);
...
(async () => {
  try {
    ...
    const compileAssets = (mode) => {
      ...
      // Write app config
      const configPath = path.join(rootDir, 'dist', 'app-config.json');
      fs.writeFileSync(configPath, JSON.stringify({ mode }, null, 2));
      console.log(`Wrote dist/app-config.json for mode: ${mode}.`);

      // Copy mode-specific bundled icon into dist for offline, zero-network desktop app rendering
      const bundledIconSrc = path.join(rootDir, 'src', 'assets', 'images', mode.toLowerCase(), 'icon.png');
      const bundledIconDest = path.join(rootDir, 'dist', 'app-icon.png');
      if (fs.existsSync(bundledIconSrc)) {
        fs.copyFileSync(bundledIconSrc, bundledIconDest);
        console.log(`Copied bundled ${mode} icon to dist/app-icon.png for desktop packaging.`);
      }
    };
...
`;
    ...
    const packageApp = async (mode) => {
      ...
      const buildIconDir = path.join(rootDir, 'build');
      if (!fs.existsSync(buildIconDir)) {
        fs.mkdirSync(buildIconDir, { recursive: true });
      }

      const modeKey = mode.toLowerCase();
      const modeDir = path.join(rootDir, 'src', 'assets', 'images', modeKey);
      const modeIconPath = path.join(modeDir, 'icon.png');
      const placeholderPath = path.join(rootDir, 'src', 'assets', 'images', 'default', 'icon.png');
      ...
      if (fs.existsSync(buildIconsetSubdir)) fs.rmSync(buildIconsetSubdir, { recursive: true, force: true });
      fs.mkdirSync(buildIconsetSubdir, { recursive: true });

      // All 10 standard iconset frames required by Apple's iconutil specification
      const macIconFramesMapping = [
        { dest: 'icon_512x512@2x.png', w: 1024, h: 1024, candidates: ['1024x1024.png', '512x512@2x.png', '1024.png', 'icon_512x512@2x.png', 'icon_1024x1024.png', 'icon_1024.png'] },
        { dest: 'icon_512x512.png', w: 512, h: 512, candidates: ['512x512.png', '512.png', 'icon_512x512.png', 'icon_512.png'] },
        { dest: 'icon_256x256@2x.png', w: 512, h: 512, candidates: ['512x512.png', '256x256@2x.png', '512.png', 'icon_256x256@2x.png', 'icon_512x512.png', 'icon_512.png'] },
        { dest: 'icon_256x256.png', w: 256, h: 256, candidates: ['256x256.png', '256.png', 'icon_256x256.png', 'icon_256.png'] },
        { dest: 'icon_128x128@2x.png', w: 256, h: 256, candidates: ['256x256.png', '128x128@2x.png', '256.png', 'icon_128x128@2x.png', 'icon_256x256.png', 'icon_256.png'] },
        { dest: 'icon_128x128.png', w: 128, h: 128, candidates: ['128x128.png', '128.png', 'icon_128x128.png', 'icon_128.png'] },
        { dest: 'icon_32x32@2x.png', w: 64, h: 64, candidates: ['64x64.png', '32x32@2x.png', '64.png', 'icon_32x32@2x.png', 'icon_64x64.png', 'icon_64.png'] },
        { dest: 'icon_32x32.png', w: 32, h: 32, candidates: ['32x32.png', '32.png', 'icon_32x32.png', 'icon_32.png'] },
        { dest: 'icon_16x16@2x.png', w: 32, h: 32, candidates: ['32x32.png', '16x16@2x.png', '32.png', 'icon_16x16@2x.png', 'icon_32x32.png', 'icon_32.png'] },
        { dest: 'icon_16x16.png', w: 16, h: 16, candidates: ['16x16.png', '16.png', 'icon_16x16.png', 'icon_16.png'] }
      ];
      ...
      for (const frame of macIconFramesMapping) {
        ...
        if (fs.existsSync(macIconDir)) {
          for (const cand of frame.candidates) {
            ...
            if (fs.existsSync(candPath) && fs.statSync(candPath).size > 100) {
              const dims = getPngDimensions(candPath);
              if (dims && dims.width === frame.w && dims.height === frame.h) {
        ...
        const destPath = path.join(buildIconsetSubdir, frame.dest);

        if (foundSource) {
          fs.copyFileSync(foundSource, destPath);
          fs.copyFileSync(foundSource, path.join(buildIconsSubdir, frame.dest));
        } else if (process.platform === 'darwin' && fs.existsSync(chosenIconSource)) {
          // Downscale cleanly using macOS native sips to avoid palette/color corruption in small icons
          try {
            execSync(`sips -z ${frame.h} ${frame.w} "${chosenIconSource}" --out "${destPath}"`, { stdio: 'ignore' });
            fs.copyFileSync(destPath, path.join(buildIconsSubdir, frame.dest));
          } catch (sipsErr) {
            console.warn(`sips downscaling skipped for ${frame.dest}:`, sipsErr.message);
          }
        }
      }
      ...
      // If running on macOS host, ALWAYS compile build/icon.icns directly via Apple's native iconutil
      // This embeds true 32-bit RGBA PNG streams for 16x16/32x32, completely bypassing third-party 8-bit palette bugs
      let compiledWithIconutil = false;
      if (process.platform === 'darwin') {
        try {
          console.log(`Compiling pristine build/icon.icns via native Apple iconutil from build/icon.iconset for ${mode}...`);
          execSync(`iconutil -c icns "${buildIconsetSubdir}" -o "${path.join(buildIconDir, 'icon.icns')}"`, { stdio: 'inherit' });
          if (fs.existsSync(path.join(buildIconDir, 'icon.icns')) && fs.statSync(path.join(buildIconDir, 'icon.icns')).size > 1000) {
            compiledWithIconutil = true;
            console.log(`Successfully generated Apple-certified build/icon.icns via iconutil for ${mode}.`);
          }
        } catch (iconutilErr) {
      ...
      // Fallback: If not on macOS runner or iconutil was unavailable, copy pre-generated icon.icns if valid
      if (!compiledWithIconutil) {
        if (isRealIconFile(macIconSource)) {
          try {
            fs.copyFileSync(macIconSource, path.join(buildIconDir, 'icon.icns'));
            console.log(`Copied pre-generated ${modeKey}/macos/icon.icns to build/icon.icns for macOS.`);
          } catch (err) {
      ...
      pkg.build.win.executableName = `AMP ${mode}`;

      // Configure Windows specific artifactNames and isolated NSIS GUIDs dynamically
      const modeNsisGuids = {
        'Admin': 'e38c92bf-79b8-4e12-b5e1-000000000001',
        'Live': 'e38c92bf-79b8-4e12-b5e1-000000000002',
        'Studio': 'e38c92bf-79b8-4e12-b5e1-000000000003'
      };

      if (!pkg.build.nsis) pkg.build.nsis = {};
      pkg.build.nsis.guid = modeNsisGuids[mode] || `e38c92bf-79b8-4e12-b5e1-${mode.toLowerCase()}`;
      ...
      pkg.build.afterAllArtifactBuild = "./scripts/afterAllArtifactBuild.cjs";
    ...
    restorePkg();
    const backupReleaseDir = path.join(rootDir, 'release_backup_temp');
