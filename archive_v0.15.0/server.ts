...
// Unified path resolution helpers
...
function getAgateMediaAnnouncementsDir(settings: ServerSettings = currentSettings): string | null {
  ...
  if (settings.localPathAgate && fs.existsSync(settings.localPathAgate)) {
    ...
    return p1; // default relative path
  }
...
function getCalendarFilePath(): string | null {
  ...
  if (settingsDir && fs.existsSync(settingsDir)) {
    const targetFile = path.join(settingsDir, 'interstitials.json');
...
function getLogBackupPath(): string | null {
  const logsDir = getAgateLogsDir();
  if (logsDir && fs.existsSync(logsDir)) {
    return path.join(logsDir, 'loghistory', 'logs_backup.json');
  }
  return null;
}
...
function getShowsFilePath(): string | null {
  ...
  const settingsDir = getAgateSettingsDir();
  if (settingsDir && fs.existsSync(settingsDir)) {
    return path.join(settingsDir, 'shows.json');
  }
  return null;
}
...
<body>
  ...
  app.post('/api/check-local-paths', (req, res) => {
    try {
      const { localPathAgate, localPathMP3s, localPathLogs, localPathCalendar } = req.body;
      
      if (localPathAgate) {
        const agateExists = fs.existsSync(localPathAgate);
        const logsDir = path.join(localPathAgate, 'logs');
        const settingsDir = path.join(localPathAgate, 'settings');
        const mediaAnnounce = path.join(localPathAgate, 'media_announcements');
        const mediaEvergreen = path.join(localPathAgate, 'media_evergreens');
        const mediaShows = path.join(localPathAgate, 'media_shows');

        const allDirsExist = agateExists &&
          fs.existsSync(logsDir) &&
          fs.existsSync(settingsDir) &&
          fs.existsSync(mediaAnnounce) &&
          fs.existsSync(mediaEvergreen) &&
          fs.existsSync(mediaShows);

        return res.json({
          exists: agateExists,
          allDirsExist,
          agateExists,
          logsExists: fs.existsSync(logsDir),
          schedExists: fs.existsSync(settingsDir),
          mp3Exists: fs.existsSync(mediaAnnounce),
          interstitialsFolderExists: fs.existsSync(mediaAnnounce),
        });
      }
  ...
  app.post('/api/create-local-paths', (req, res) => {
    try {
      ...
      if (localPathAgate) {
        if (!fs.existsSync(localPathAgate)) {
          fs.mkdirSync(localPathAgate, { recursive: true });
          createdCount++;
        }
        const subDirs = [
          path.join(localPathAgate, 'logs'),
          path.join(localPathAgate, 'logs', 'loghistory'),
          path.join(localPathAgate, 'settings'),
          path.join(localPathAgate, 'settings', 'backups'),
          path.join(localPathAgate, 'media_announcements'),
          path.join(localPathAgate, 'media_evergreens'),
          path.join(localPathAgate, 'media_shows'),
        ];
        subDirs.forEach(dir => {
          if (!fs.existsSync(dir)) {
            try {
              ...
              console.error(`Failed to create AGATE folder ${dir}:`, err);
            }
          }
        });

        // Seed default seed files if missing
        const logsFile = path.join(localPathAgate, 'logs', 'logs.json');
        const interstitialsFile = path.join(localPathAgate, 'settings', 'interstitials.json');
        const showsFile = path.join(localPathAgate, 'settings', 'shows.json');
        ...
        return res.json({ success: true, createdCount, interstitialsReadonlyError: false, interstitialsReadonlyMessage: '' });
      }
  ...
  app.post('/api/startup/verify', (req, res) => {
    try {
      ...
      if (agateRoot) {
        ...
        // Initialize seed JSON files if missing
      ...
      // Legacy fallback: individual local paths
      ...
      // First-run files initialization for legacy paths
