# Playlist & Evergreen Folder Disambiguation Specification (v0.16.1)
...
4. **Dedicated "Pick Playlist" / "Pick Evergreen" Toolbar Controls**:
   - In Playlist, Prerecord, and Live views: Provides a **"Pick Playlist"** action button alongside **"Pick Show"**.
   - In Export view: Targets the station Evergreens directory with a dedicated **"Pick Evergreen"** action button.
   - In standard Player > Play view: Omitted to maintain a streamlined broadcast interface.
5. **Narrow-Viewport Responsive Layout**: On narrow viewports, the **"Pick Show"** control is stacked directly beneath **"Pick Playlist"** / **"Pick Evergreen"** to avoid horizontal collision and truncation.
6. **Context-Aware Folder Chooser Modal (`FolderChooserModal.tsx`)**:
   - **Found Directory / Directories**: Discovered internal show/Evergreen folders with file counts, `(current)` active badge, and `(missing)` disconnected badges.
   ...
   - **No Playlist / No Evergreen**: Loads the show with an empty audio track queue while preserving all scheduled station breaks, announcements, and live reads.
7. **Synchronous Pre-Caching & ID3 Tag Resolution**:
   - In accordance with broadcast stability requirements, all audio file pre-caching, ID3 tag extraction (`readMp3ID3Metadata`), and initial `.json` show log evaluation complete before the UI dismisses the loading indicator and renders track cards.
   - Guarantees full duration accuracy, metadata readiness, and cache presence before playback begins.
8. **Show Switching Isolation**:
   - Switching between shows or play modes completely purges stale track arrays, playback markers, cancellation registries, and cached metadata before loading. If the new show lacks an existing `.json` log, it initializes in a clean slate rather than inheriting ghost tracks from the previous show.
9. **Strict Missing Folder Verification**:
   - If a configured default directory or subfolder no longer exists on disk at startup or during configuration saves, the system does not automatically create empty directories. Instead, the operator is prompted via the Storage Folders (`LocationsModal.tsx`) interface to browse and select a valid existing directory.
10. **M3U Precedence with Loose File Append**: If an `.m3u` file exists in the selected folder, its referenced files are queued first in sequence; any remaining loose audio files in that same folder are appended to the end of the queue.
11. **Compliance History Preservation & Queue Reconciliation**: Switching folders preserves all existing entries in `playedTracks` for FCC/station broadcast compliance logs while reconciling `unplayedTrackIds` and `trackMetadata` against the new folder.
12. **Live Read / Show Picker Modal Badge**: Displays `"xx files in yy folders"` whenever multiple non-empty folders exist.
...
### 4.1 "Pick Playlist" / "Pick Evergreen" & "Pick Show" Header Controls
* **Terminology**:
  - Show selector action button: **`Pick Show`**.
  - Folder chooser action button: **`Pick Playlist`** (in `Playlist`, `Prerecord`, and `Live` views) or **`Pick Evergreen`** (in `Export` view).
  - Standard `Player > Play` view: The folder chooser action is removed to keep the live broadcast interface uncluttered.
* **Layout & Placement**:
  - Wide Viewports: **`Pick Show`** is placed directly adjacent to **`Pick Playlist`** / **`Pick Evergreen`**.
  - Narrow Viewports: **`Pick Show`** is positioned directly underneath **`Pick Playlist`** / **`Pick Evergreen`** to preserve clear touch/click targets and eliminate label clipping.
* **Interaction**: Clicking opens `FolderChooserModal` configured with the relevant show context and `folderType` (`"show"` or `"evergreen"`).

### 4.2 Context-Aware Folder Chooser Modal (`FolderChooserModal.tsx`)
* **Header**: Title: `"Choose Playlist Folder"` (or `"Choose Evergreen Folder"` when in Export mode), Subtitle: `Show: [Show Name]`.
* **Section 1: Discovered Station Folders (Radio Group)**
  - Lists every non-empty folder discovered on disk (Top Level Root and subfolders) under `/medialibrary/Playlists/<ShowName>/` or `/medialibrary/Evergreens/<ShowName>/`.
  - Each item displays:
...
* **Section 3: No Playlist / No Evergreen**
  - Selectable radio card: `"No Playlist"` (or `"No Evergreen"` in Export mode).
  - Description: `"Load show with an empty track queue"` (or `"Load without evergreen audio"` in Export mode).
* **Footer Actions**:
  - `Cancel` button (closes modal without changing active session).
  - `Load Playlist` / `Load Evergreen` primary action button (executes folder selection and applies changes).
...
## 5. Lifecycle, Re-entry, Non-Blocking Loading & Session Isolation
...
[User Clicks Show/Date in Show Picker]
               │
               ▼
[Purge Stale Show State & Track Arrays]
               │
               ▼
...
[.json Log Exists]   [No .json Log]
      │                 │
      ├─► Has Saved Folder ──► Pre-cache audio, evaluate ID3 metadata & save initial log
      │                        (Display track cards & dismiss spinner once complete)
      │
      └─► No Saved Folder  ──► Open Top Level (Root) `"."`
                               (Pre-cache audio, evaluate ID3 metadata & save initial log)
```

### 5.2 Complete Pre-Caching & ID3 Tagging Prior to Display
* In accordance with broadcast stability requirements, all audio file pre-caching, ID3 tag extraction (`readMp3ID3Metadata`), and initial `.json` show log reconciliation must complete before the UI dismisses the loading indicator (`isPlaylistLoading = false`) and displays track cards.
* Ensures that track durations, artist/title metadata, tooltips, and file availability are fully resolved and guaranteed in memory prior to playback or card interaction.

### 5.3 Show Switching Isolation
* When switching between different shows or toggling modes:
  - Active track lists, cancellation registries, and cached metadata are purged before loading the new show.
  - If the new show lacks an existing `.json` log, all state containers are cleared completely (`playedPlaylistTracks = {}`, `cancelledTrackIds = []`, `trackMetadata = {}`), preventing ghost tracks from the previous show from leaking into the new show.

### 5.4 Missing Folder Policy
* At startup or when saving folder configurations:
  - If configured folders or default directories are missing on the filesystem, the backend returns an inaccessible status.
  - The application refrains from automatically creating missing media folders.
  - The operator is prompted via `LocationsModal` to pick a valid existing folder.

### 5.5 Folder Switching Reconciliation & Played Track Retention Flow
When the broadcaster opens the folder chooser dialog and confirms a switch from Folder A to Folder B on an existing show session, or reloads a show where files differ:

1. **Preserve Compliance History & Retain Played Track Cards**:
   - The `playedTracks` array is **never wiped or overwritten**. All tracks played earlier in the broadcast session remain in `playedTracks` with their authentic metadata, ID3 tags, and timestamps.
   - Played track cards remain rendered in the active queue in chronological order, even if the underlying audio file is no longer present in the newly selected folder.
2. **Non-Colliding Sequential ID Assignment**:
   - The system finds the maximum existing numeric index (`maxIndex`) across all retained played tracks and existing log metadata.
   - Newly scanned or added tracks receive new unique IDs continuing from `maxIndex + 1` (e.g. `playlist-track-4`, `playlist-track-5`), guaranteeing that new unplayed tracks never inherit or collide with historical played IDs.
3. **Reconcile Active Queue**:
   ...
   - For each track in `existingLog.unplayedTrackIds`:
     ...
     - If not found in Folder B, prunes the unplayed track ID from `unplayedTrackIds` and `trackMetadata`.
   - For newly discovered files in Folder B not in `existingLog`:
     - Assigns new sequential IDs continuing from `maxIndex + 1`.
     - Appends IDs to the end of `unplayedTrackIds`.
     - Inserts metadata into `trackMetadata`.
4. **If "No Playlist" / "No Evergreen" is Selected**:
   - Retains all historical `playedTracks` and their rendered cards.
