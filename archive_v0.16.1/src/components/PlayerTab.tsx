...
export default function PlayerTab({ 
  ...
  const handleSwitchPlaylistFolder = async (newFolder: {
    ...
    try {
      ...
      const newNormalizedTracks = (serverTracks as any[]).map((t: any, idx: number) => {
        const id = t.id || `playlist-track-${idx + 1}`;
        return { ...t, id };
      });
  ...
  const playlistTimeline = useMemo(() => {
    ...
    // 3. Separate Missed Announcement Breaks vs Upcoming Announcement Breaks
    // Rule 4.a: Missed announcements sort AFTER played items and BEFORE the NOW indicator
