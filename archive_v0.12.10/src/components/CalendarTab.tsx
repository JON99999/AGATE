  ...
  const handleConfirmPlacementModal = () => {
    ...
    if (placementModal.transitionMp3Id && startIso) {
      updatedCurrent = updatedCurrent.map(m =>
        m.id === placementModal.transitionMp3Id ? { ...m, endDate: startIso } : m
      );
    } else {
      ...
      if (last && (!last.endDate || !last.endDate.trim()) && startIso > (last.startDate || '')) {
        updatedCurrent = updatedCurrent.map(m =>
          m.id === last.id ? { ...m, endDate: startIso } : m
        );
      }
    }
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-stretch w-full shadow-xs rounded-lg overflow-hidden border border-slate-350 focus-within:ring-1 focus-within:ring-blue-500 focus-within:border-blue-500 bg-white">
                  ...
                  <input 
                    ...
                    className={cn(
                      "w-full px-2.5 py-1 text-xs font-black text-slate-800 bg-white outline-none",
                      !formData.name && editingId ? "bg-red-50/50" : ""
                    )}
                  />
                </div>
                {!formData.name && <p className="text-[10px] text-red-500 font-bold uppercase tracking-tighter mt-0.5">Name is required</p>}
              </div>
                          ...
                          {validation.sorted.map((item, idx) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                {/* Starting Gap Indicator above Start in the top of the card */}
                                {startGapWarning && (
                                  <div className="flex items-center gap-1.5 text-xs font-bold text-amber-700 bg-amber-50 border border-amber-200 rounded px-2 py-0.5 mb-1 select-none">
                                    <AlertCircle className="w-3.5 h-3.5 shrink-0 text-amber-500" />
                                    <span>{startGapWarning}</span>
                                  </div>
                                )}
                                ...
                                <div className="flex flex-col md:flex-row gap-2 items-start">
                                  ...
                                  <div className="flex-1 min-w-0 space-y-1 w-full">
                                      ...
                                      <div className="flex items-center gap-1.5 shrink-0">
                                        ...
                                        <div className="flex items-center gap-0 shrink-0">
                                          ...
                                          <button
                                            ...
                                            title="Choose MP3 file from disk"
                                          >
                                    ...
                                    {(() => {
                                      const bottomWarnings = startGapWarning 
                                        ? itemWarnings.filter(w => w !== startGapWarning) 
                                        : itemWarnings;
