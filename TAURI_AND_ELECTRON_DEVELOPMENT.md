...
- **Electron Outputs**: Saved as standard desktop installers/portable files inside `/release/` (e.g., `Interstitial-er Player-0.8.9-Windows-Portable.exe`).
- **Tauri Outputs**: Saved inside a dedicated subfolder `/release/tauri/` with `-tauri` appended to their file names to avoid collisions (e.g., `Interstitial-er Player-0.8.9-Mac-Silicon-new-tauri.dmg`).
...
Both build pipelines are also fully managed in GitHub Actions on every official tag release (e.g., push `v0.8.9`) inside the `.github/workflows/release.yml` pipeline.
