...
type SortField = 'timestamp' | 'mp3Name' | 'announcementName' | 'announcementId' | 'playMode' | 'logTimeStamp' | 'showName' | 'hostName';
...
function formatLogTime(dateVal: string | number | Date, width: number): { dateStr: string; timeStr: string } {
  ...
  let timeStr = `${hh12}:${minStr}:${secStr} ${ampm}`;

  // 1. First, Truncate "hh:mm:ss AM" to "hh:mm AM"
  if (width < 220) {
    timeStr = `${hh12}:${minStr} ${ampm}`;
  }

  // 2. Then, to "h:mm AM" with no leading zero on the h
  if (width < 200) {
...
export default function LogTab({ logs }: LogTabProps) {
  ...
  const filteredLogsBase = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      result = result.filter(l => {
        try {
          const announcementNameMatch = l.announcementName && l.announcementName.toLowerCase().includes(q);
          const filenameMatch = getMP3Status(l.mp3Name).filename.toLowerCase().includes(q);
          const idMatch = l.announcementId && l.announcementId.toLowerCase().includes(q);
          ...
          const logFormatted = formatLogTime(l.timestamp, 300);
          const timestampMatch = format(new Date(l.timestamp), 'yyyy-MM-dd HH:mm:ss').includes(q) ||
            format(new Date(l.timestamp), 'yyyy-MM-dd h:mm:ss a').toLowerCase().includes(q) ||
            logFormatted.timeStr.toLowerCase().includes(q) ||
            logFormatted.dateStr.toLowerCase().includes(q);
          const actualFormatted = l.logTimeStamp ? formatLogTime(l.logTimeStamp, 300) : null;
          const actualTimestampMatch = l.logTimeStamp && (
            format(new Date(l.logTimeStamp), 'yyyy-MM-dd HH:mm:ss').includes(q) ||
            format(new Date(l.logTimeStamp), 'yyyy-MM-dd h:mm:ss a').toLowerCase().includes(q) ||
            (actualFormatted ? actualFormatted.timeStr.toLowerCase().includes(q) || actualFormatted.dateStr.toLowerCase().includes(q) : false)
          );
          const showMatch = l.showName && l.showName.toLowerCase().includes(q);
          const hostMatch = l.hostName && l.hostName.toLowerCase().includes(q);
          
          return announcementNameMatch || filenameMatch || idMatch || playModeMatch || assetTypeMatch || timestampMatch || actualTimestampMatch || showMatch || hostMatch;
        } catch (e) {
          return (l.announcementName && l.announcementName.toLowerCase().includes(q)) || (l.announcementId && l.announcementId.toLowerCase().includes(q));
        }
      });
    }
  ...
  const handleExportCSV = () => {
    const headers = ['Scheduled Date', 'Scheduled Time', 'Actual Playback Time', 'Announcement Name', 'MP3 File', 'Show Name', 'Host Name', 'Play Mode', 'Announcement ID', 'Asset Type'];
    const rows = sortedAndFilteredLogsAll.map(log => [
      format(new Date(log.timestamp), 'yyyy-MM-dd'),
      format(new Date(log.timestamp), 'hh:mm:ss a'),
      log.logTimeStamp ? format(new Date(log.logTimeStamp), 'yyyy-MM-dd hh:mm:ss a') : '-',
      log.announcementName,
      log.mp3Name,
      log.showName || '',
      log.hostName || '',
      log.playMode || 'Live',
    ...
    link.href = url;
    link.setAttribute('download', `interstititaler_logs_${format(new Date(), 'yyyyMMdd_hhmmss_a')}.csv`);
  ...
  const handleExportXLSX = () => {
    const exportData = sortedAndFilteredLogsAll.map(log => ({
      ...
      'Scheduled Time': format(new Date(log.timestamp), 'hh:mm:ss a'),
      'Actual Playback Time': log.logTimeStamp ? format(new Date(log.logTimeStamp), 'yyyy-MM-dd hh:mm:ss a') : '-',
      'Announcement Name': log.announcementName,
      'MP3 File': log.mp3Name,
      'Show Name': log.showName || '',
      'Host Name': log.hostName || '',
      'Play Mode': log.playMode || 'Live',
      'Announcement ID': log.announcementId,
    ...
    XLSX.writeFile(wb, `interstititaler_logs_${format(new Date(), 'yyyyMMdd_hhmmss_a')}.xlsx`);
  };
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {isLogStacked ? (
            <div className="bg-slate-100 border-b border-grid-active p-2 flex flex-col gap-1 text-xs font-black text-slate-650 uppercase tracking-wider shrink-0 select-none sticky top-0 z-20 shadow-sm border-l-4 border-l-transparent">
              {containerWidth < 325 ? (
                ...
                <div className="flex flex-wrap items-center gap-x-1.5 gap-y-1 min-w-0 w-full text-[11px]">
                  ...
                  <span className="text-slate-300 font-normal">:</span>
                  <button 
                    onClick={() => toggleSort('announcementId')}
                    className="flex items-center gap-0.5 cursor-pointer hover:text-slate-900 transition-colors text-slate-650"
                    title="Sort by Announcement ID"
                  >
                    <span>ID#</span>
                    <SortArrow field="announcementId" />
                  </button>
                  <span className="text-slate-300 font-normal">:</span>
                  <button 
            ...
            <div className="bg-slate-100 border-b border-grid-active py-0.5 flex items-stretch text-xs font-black text-slate-650 uppercase tracking-wider shrink-0 select-none sticky top-0 z-20 shadow-sm border-l-4 border-l-transparent">
              ...
              {/* 2nd Column: Consolidated Announcement & MP3 / Show & Host details */}
              <div className="flex-1 flex flex-col justify-center py-0.5 gap-0.5 px-2 min-w-0 overflow-hidden">
                {/* Top row sorting: Announcement Name : MP3 File */}
              ...
              {/* 4th Column: Announcement ID & Play Mode */}
          ...
          {displayedLogs.length > 0 ? (
            displayedLogs.map((log, i) => {
              ...
              if (isLogStacked) {
                return (
                  <div 
                    key={`${log.announcementId}-${log.timestamp}-${i}`}
                    className={cn(
                      "flex flex-col p-2 gap-1 border-b border-grid-active hover:bg-slate-50 transition-colors grow",
                    ...
                    {containerWidth < 375 ? (
                      <div className="text-xs leading-tight min-w-0 w-full line-clamp-2 break-all">
                        <span className="inline-flex items-center gap-1 font-bold text-slate-850 align-baseline mr-1">
                          ...
                          <span title={log.announcementName}>{log.announcementName || '—'}</span>
                        </span>
                      ...
                      <div className="flex flex-col gap-0 min-w-0">
                        {/* Top row: Announcement Name : MP3 File */}
                        <div className="flex items-center gap-1.5 min-w-0 w-full truncate leading-tight">
                          ...
                          <span className="text-xs font-bold text-slate-850 truncate shrink-0 max-w-[50%]" title={log.announcementName}>
                            {log.announcementName || '—'}
                          </span>
              ...
              return (
                <div 
                  key={`${log.announcementId}-${log.timestamp}-${i}`}
                  className={cn(
                    "flex items-stretch border-b border-grid-active hover:bg-slate-50 transition-colors grow min-h-[32px]",
                    i % 2 === 0 ? "bg-white" : "bg-slate-205",
                    getLogAssetType(log) === 'script' && "border-l-4 border-l-purple-500",
                    getLogAssetType(log) === 'audio' && "border-l-4 border-l-blue-500"
                  )}
                >
                  ...
                  <div style={{ width: `${colWidths.time}px` }} className="text-xs font-mono font-bold text-slate-900 tabular-nums flex flex-col justify-center gap-0 pr-1 pl-2 shrink-0 overflow-hidden py-1">
                    <div className="leading-tight line-clamp-2 text-ellipsis overflow-hidden text-slate-905" title={`${format(new Date(log.timestamp), 'yyyy-MM-dd')} ${format(new Date(log.timestamp), 'hh:mm:ss a')}`}>
                      S:{sched.dateStr} {sched.timeStr}
                    </div>
                    {actl ? (
                      <span 
                        ...
                        title={`ACTL: ${format(new Date(log.logTimeStamp), 'yyyy-MM-dd hh:mm:ss a')}`}
                      >
                        A:{actl.dateStr} {actl.timeStr}
                      </span>
                    ) : (
                  ...
                  {/* Consolidated Announcement Name: MP3 File on top row, Show Name: Host Name on bottom row */}
