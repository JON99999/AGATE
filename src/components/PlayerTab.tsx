...
export default function PlayerTab({ schedules, logs, onLog, now, syncTime, scrollTrigger }: PlayerTabProps) {
  ...
  const handlePlay = (s: Schedule, slot: Date) => {
    ...
    const audio = new Audio(s.mp3Url);
    ...
    <div className="flex flex-col h-full">
      ...
      >
        {timeline.map((slot) => {
          ...
          if (sForSlot.length === 0 && !isPresent) return null;
            ...
            <div key={slot.toISOString()} className="space-y-2">
              {isPresent && (
                <div 
                  ...
                  id="now-indicator"
                >
                  <span className="text-[10px] font-black uppercase text-white tracking-widest">now</span>
                </div>
              )}
              ...
              {sForSlot.map((s, idx) => {
                ...
                return (
                  <div 
                    ...
                    className={cn(
                      ...
                      isCurrentlyPlaying || isUpcoming ? "border-blue-500 ring-1 ring-blue-500/20" : "border-slate-200",
                      isUpcoming ? "bg-blue-50/20" : "",
                      ...
                      isPresent && !isCurrentlyPlaying ? "bg-blue-50/30 border-blue-200" : "",
                ...
                <div className="flex justify-between items-center bg-slate-50 -mx-2 -mt-2 px-2 py-1 rounded-t">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={cn(
                      ...
                      isMissedRecent && !isCurrentlyPlaying ? "text-orange-900" : (isPresent || isCurrentlyPlaying || isUpcoming) ? "text-blue-600" : "text-slate-900"
                    )}>
                      {format(slot, 'HH:mm')}
                    </span>
                  </div>
                  {isCurrentlyPlaying ? (
                    <div className="flex items-center gap-1 text-[8px] font-black uppercase text-blue-600 animate-pulse">
                      <div className="w-1.5 h-1.5 rounded-full bg-blue-600"></div>
                      Live
                    </div>
                  ) : isPresent ? (
                    <span className="text-[8px] bg-blue-600 text-white px-1 py-0.5 rounded font-black uppercase leading-none">Next</span>
                  ) : isUpcoming ? (
                    <span className="text-[8px] bg-blue-500 text-white px-1 py-0.5 rounded font-black uppercase leading-none shadow-sm animate-pulse">Upcoming</span>
                  ) : null}
                </div>
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  <button 
                    ...
                    className={cn(
                      ...
                      isPresent || isUpcoming ? "bg-blue-600 text-white shadow-md shadow-blue-200" :
                ...
                <div className="flex items-center justify-between">
                  ...
                  {isCurrentlyPlaying ? (
                    <div className="flex items-center gap-1 text-[8px] font-mono font-bold text-blue-600 leading-none">
