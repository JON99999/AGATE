...
// Unified path resolution helpers for strict 5-folder AGATE structure:
...
function getAgateLogHistoryDir(settings: ServerSettings = currentSettings): string | null {
  if (settings.paths?.loghistory && fs.existsSync(settings.paths.loghistory)) {
    return settings.paths.loghistory;
  }
  const logsDir = getAgateLogsDir(settings);
  ...
  if (settings.localPathAgate && fs.existsSync(settings.localPathAgate)) {
    const p1 = path.join(settings.localPathAgate, 'LogHistory');
    if (fs.existsSync(p1)) return p1;
    const p2 = path.join(settings.localPathAgate, 'loghistory');
    if (fs.existsSync(p2)) return p2;
  }
  return null;
}
...
function getAgateSettingsDir(settings: ServerSettings = currentSettings): string | null {
  ...
  if (settings.localPathAgate && fs.existsSync(settings.localPathAgate)) {
    ...
    if (fs.existsSync(p1)) return p1;
    const p2 = path.join(settings.localPathAgate, 'Settings');
    if (fs.existsSync(p2)) return p2;
    return p1;
  }
...
function getAgateMediaAnnouncementsDir(settings: ServerSettings = currentSettings): string | null {
  ...
  if (settings.localPathAgate && fs.existsSync(settings.localPathAgate)) {
    ...
    if (fs.existsSync(p1)) return p1;
    const p2 = path.join(settings.localPathAgate, 'MediaAnnouncements');
    if (fs.existsSync(p2)) return p2;
    return p1;
  }
  return null;
...
function getAgateMediaEvergreensDir(settings: ServerSettings = currentSettings): string | null {
  ...
  if (settings.localPathAgate && fs.existsSync(settings.localPathAgate)) {
    ...
    if (fs.existsSync(p1)) return p1;
    const p2 = path.join(settings.localPathAgate, 'MediaEvergreens');
    if (fs.existsSync(p2)) return p2;
    return p1;
  }
  return null;
...
function getAgateMediaShowsDir(settings: ServerSettings = currentSettings): string | null {
  ...
  if (settings.localPathAgate && fs.existsSync(settings.localPathAgate)) {
    ...
    if (fs.existsSync(p1)) return p1;
    const p2 = path.join(settings.localPathAgate, 'MediaShows');
    if (fs.existsSync(p2)) return p2;
    return p1;
  }
  return null;
...
function getCalendarFilePath(): string | null {
  ...
  if (currentSettings.paths?.interstitialsFile && fs.existsSync(currentSettings.paths.interstitialsFile)) {
    return currentSettings.paths.interstitialsFile;
  }
  // 2. Settings directory (unified localPathAgate or legacy localPathCalendar)
  const settingsDir = getAgateSettingsDir();
  if (settingsDir && fs.existsSync(settingsDir)) {
    let targetFile = path.join(settingsDir, 'interstitials.json');
    if (!fs.existsSync(targetFile)) {
      const capFile = path.join(settingsDir, 'Interstitials.json');
      if (fs.existsSync(capFile)) {
        return capFile;
      }
      // Check potential migration locations
      const subFolderFile = path.join(settingsDir, 'Interstitials', 'interstitials.json');
      const mediaDir = getAgateMediaAnnouncementsDir();
      const mediaSubFolderFile = mediaDir && fs.existsSync(mediaDir)
        ? path.join(mediaDir, 'interstitials.json')
        : null;
...
function getCalendarBackupPath(): string | null {
  const settingsDir = getAgateSettingsDir();
  if (settingsDir && fs.existsSync(settingsDir)) {
    return path.join(settingsDir, 'backups', 'interstitials_backup.json');
  }
  return null;
}
...
<body>
  ...
  const handleStartupVerify = (req: express.Request, res: express.Response) => {
    try {
      ...
      if (agateRoot) {
        ...
        const logsDir = getAgateLogsDir(settings) || path.join(agateRoot, 'logs');
        const logHistoryDir = getAgateLogHistoryDir(settings) || path.join(logsDir, 'loghistory');
        const settingsDir = getAgateSettingsDir(settings) || path.join(agateRoot, 'settings');
        const settingsBackupsDir = path.join(settingsDir, 'backups');
        const mediaAnnounceDir = getAgateMediaAnnouncementsDir(settings) || path.join(agateRoot, 'media_announcements');
        const mediaEvergreensDir = getAgateMediaEvergreensDir(settings) || path.join(agateRoot, 'media_evergreens');
        const mediaShowsDir = getAgateMediaShowsDir(settings) || path.join(agateRoot, 'media_shows');
        ...
        return res.json({
          ready: true,
          status: 'READY',
          message: 'AGATE Station Location and 5-folder structure verified successfully.'
        });
      }

      // --- STRATEGY 2: Individual 5-Folder Path Overrides ---
      const settingsDir = getAgateSettingsDir(settings);
      const logsDir = getAgateLogsDir(settings);
      const mediaAnnounceDir = getAgateMediaAnnouncementsDir(settings);
      const mediaEvergreensDir = getAgateMediaEvergreensDir(settings);
      const mediaShowsDir = getAgateMediaShowsDir(settings);
      ...
      return res.json({
        ready: true,
        status: 'READY',
        message: 'Configured 5-folder AGATE structure verified successfully.'
      });
    } catch (e: any) {
  ...
  app.get('/api/local-mp3s', (req, res) => {
    try {
      const targetDir = getAgateMediaAnnouncementsDir(currentSettings);
  ...
  function resolveMediaFilePath(query: any): string | null {
    ...
    if (mediaShows && fs.existsSync(mediaShows) && file) {
      ...
      if (fs.existsSync(p) && fs.statSync(p).isFile()) return p;
      const resolved = findShowSourceFile(mediaShows, file, showNameShort, showName) || findInterstitialSourceFile(mediaShows, file);
      if (resolved && fs.existsSync(resolved) && fs.statSync(resolved).isFile()) return resolved;
    }

    // 6. Check relative to unified AGATE station root
  ...
  app.get('/api/live-read/content', (req, res) => {
    try {
      ...
      const announcementsDir = getAgateMediaAnnouncementsDir(currentSettings);
      ...
      if (!fs.existsSync(targetFilePath)) {
        ...
        if (!fs.existsSync(targetFilePath)) {
          const resolved = findInterstitialSourceFile(announcementsDir, file);
  ...
  app.post('/api/shows/verify-evergreens', (req, res) => {
    try {
      let evergreensPath = getAgateMediaEvergreensDir(currentSettings);
      let playlistsPath = getAgateMediaShowsDir(currentSettings);
      let interstitialsPath = getAgateMediaAnnouncementsDir(currentSettings);

      if (!evergreensPath && currentSettings.localPathAgate) {
        evergreensPath = path.join(currentSettings.localPathAgate, 'media_evergreens');
      }
      if (!playlistsPath && currentSettings.localPathAgate) {
        playlistsPath = path.join(currentSettings.localPathAgate, 'media_shows');
      }
      if (!interstitialsPath && currentSettings.localPathAgate) {
        interstitialsPath = path.join(currentSettings.localPathAgate, 'media_announcements');
      }

      if (!evergreensPath || !playlistsPath || !interstitialsPath) {
        return res.status(400).json({ error: 'Media directories are not defined or offline. Please configure AGATE Station Location or Media paths in Settings.' });
      }
      ...
      if (!fs.existsSync(interstitialsPath)) {
        try {
          ...
          interstitialsReadonlyMessage = "The 'media_announcements' folder does not exist in your Media folder and could not be created because the directory is read-only. Administrator assistance is required to set folder permissions or create the 'media_announcements' folder manually.";
        }
      }
  ...
  app.post('/api/shows/evergreen/check-folder', (req, res) => {
    try {
      ...
      const evergreensPath = getAgateMediaEvergreensDir(currentSettings);
      const playlistsPath = getAgateMediaShowsDir(currentSettings);
  ...
  app.post('/api/shows/playlist/check-show-files', async (req, res) => {
    try {
      ...
      const showsPath = getAgateMediaShowsDir(currentSettings);
      const evergreensPath = getAgateMediaEvergreensDir(currentSettings);
  ...
  app.post('/api/shows/playlist/save-log-json', (req, res) => {
    try {
      ...
      const logHistoryDir = getAgateLogHistoryDir(currentSettings);
  ...
  // Helper to locate Interstitial / Announcement source file strictly inside Media Announcements
  ...
  app.post('/api/check-export-folder', (req, res) => {
    try {
      ...
      const defaultDest = getAgateMediaShowsDir(currentSettings) || (currentSettings.localPathAgate ? path.join(currentSettings.localPathAgate, 'media_shows') : null);
  ...
  app.post('/api/export-prerecord', async (req, res) => {
    try {
      ...
      const evergreensFolder = getAgateMediaEvergreensDir(currentSettings);
      const announcementsFolder = getAgateMediaAnnouncementsDir(currentSettings);
      const showsFolder = getAgateMediaShowsDir(currentSettings);
      ...
      for (let idx = 0; idx < items.length; idx++) {
        ...
        if (item.isEvergreen) {
          ...
          if (announcementsFolder) {
            sourceFilePath = findInterstitialSourceFile(announcementsFolder, sourceFileName);
          }
          if (!sourceFilePath && showsFolder) {
            sourceFilePath = findInterstitialSourceFile(showsFolder, sourceFileName);
          }
        }
        ...
        if (isScript && item.backupMp3Url) {
          ...
          const backupSourcePath = announcementsFolder ? findInterstitialSourceFile(announcementsFolder, item.backupMp3Url) : null;
