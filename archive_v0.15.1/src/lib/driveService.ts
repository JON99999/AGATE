 ...
 * In unified mode (driveFolderAgate is set), it looks for/creates 'settings' inside driveFolderAgate.
/**
 * Resolves or creates the 'settings' folder strictly inside the AGATE station root on Google Drive
...
export const getOrCreateDriveSettingsFolder = async (): Promise<string> => {
  ...
  if (!settingsId) {
    settingsId = await findFileInFolderCaseInsensitive('Settings', agateFolder);
  }
  if (!settingsId) {
    settingsId = await findFileInFolderCaseInsensitive('preferences', agateFolder);
  }
  if (!settingsId) {
...
export const getOrCreateDriveLogsFolder = async (): Promise<string> => {
  ...
  if (!logsId) {
    logsId = await findFileInFolderCaseInsensitive('Logs', agateFolder);
  }
  if (!logsId) {
...
/**
 * Resolves or creates the 'media_announcements' folder strictly inside the AGATE station root on Google Drive.
 */
export const getOrCreateDriveAnnouncementsFolder = async (): Promise<string> => {
  ...
  const rootFolder = DRIVE_FOLDERS.agate;
  if (!rootFolder) {
    throw new Error('Google Drive Station folder is not configured. Please set it in Settings.');
  }
...
/**
 * Resolves or creates the 'media_evergreens' folder strictly inside the AGATE station root on Google Drive.
 */
...
/**
 * Resolves or creates the 'media_shows' folder strictly inside the AGATE station root on Google Drive.
 */
...
/**
 * Load schedules from Drive interstitials.json in settings folder (or legacy preferences folder)
 */
export const loadCalendarFromDrive = async (): Promise<Interstitial[]> => {
  try {
    const settingsFolder = await getOrCreateDriveSettingsFolder();
    let fileId = await findFileInFolder('interstitials.json', settingsFolder);
    if (!fileId) {
      // Create empty interstitials.json if not found
      fileId = await createFileInFolder('interstitials.json', settingsFolder);
...
/**
 * Save schedules to Drive interstitials.json in settings folder
 */
export const saveCalendarToDrive = async (schedules: Interstitial[]): Promise<void> => {
  try {
    ...
    const normalizedSchedules = normalizeInterstitials(schedules);
    await uploadFileContent(fileId, JSON.stringify({ InterstitialsBackupCounter: counter, data: normalizedSchedules }, null, 2));
  } catch (err) {
...
/**
 * Lists MP3 files from Drive media_announcements (or legacy Interstitials) folder
 */
...
export const listMP3sFromDrive = async (): Promise<DriveMP3[]> => {
  try {
    const interstitialsFolderId = await getOrCreateDriveAnnouncementsFolder();
...
export interface DriveValidationResult {
  ...
  checkedFolders: {
    agate?: { id: string; ok: boolean; status?: number; name?: string };
...
export const verifyGoogleDriveDetails = async (
    ...
    if (tokenInfoRes.ok) {
      ...
      if (!hasDriveScope && scopes.includes('https://www.googleapis.com/auth/drive.appdata')) {
        return {
          ...
          tokenUser,
          checkedFolders: {},
          errorMessage: 'Token was issued with scope "drive.appdata", which only allows access to hidden app data. In Google OAuth Playground, select "https://www.googleapis.com/auth/drive" in Step 1 so Interstitial-er can read your station audio files and logs.'
        };
      }
  ...
  const agateFolder = (settings.driveFolderAgate || '').trim();
  ...
  if (agateFolder) {
    const agateRes = await checkFolder(agateFolder);
    checkedFolders.agate = { id: agateFolder, ...agateRes };
    if (!agateRes.ok) {
      const reason = agateRes.status === 404 ? 'Folder not found' : agateRes.status === 403 ? 'Access denied' : `HTTP ${agateRes.status || 'unknown'}`;
      return {
        valid: false,
        tokenValid: true,
        tokenUser,
        checkedFolders,
        errorMessage: `Main Station folder (${agateFolder}) could not be accessed (${reason}). Ensure the folder exists in Google Drive and is shared with ${tokenUser?.email || 'your account'}.`
      };
    }
  ...
  if (foldersToCheck.length === 0) {
    ...
    return {
      ...
      tokenUser,
      checkedFolders,
      warningMessage: 'Google account authenticated. Please configure the AGATE Station folder above.'
    };
  }
...
export const verifyEvergreensOnDrive = async (shows: Show[]): Promise<{
  ...
  try {
    ...
    const interstitialsId = await getOrCreateDriveAnnouncementsFolder();
    ...
    const isAgate = !!DRIVE_FOLDERS.agate;

    return {
      success: true,
      evergreensFolderCreated: false,
      playlistsFolderCreated: false,
      interstitialsFolderCreated: false,
      evergreensPath: isAgate ? 'Google Drive: [Station]/media_evergreens' : 'Google Drive: /medialibrary/Evergreens',
      playlistsPath: isAgate ? 'Google Drive: [Station]/media_shows' : 'Google Drive: /medialibrary/Playlists',
      interstitialsPath: isAgate ? 'Google Drive: [Station]/media_announcements' : 'Google Drive: /medialibrary/Interstitials',
      createdFolders
    };
  } catch (err: any) {
