...
export default function App() {
  ...
  const confirmNavAction = (action: () => void) => {
    ...
    if (
      typeof (window as any).interstitialerHasUnsavedChanges === "function" &&
  ...
  useEffect(() => {
    (window as any).interstitialerTriggerFocusRefresh = (reason?: string) => {
      triggerFocusRefresh(reason);
    };
    return () => {
      (window as any).interstitialerTriggerFocusRefresh = undefined;
    };
  }, []);
  ...
  const [localPathAgate, setLocalPathAgate] = useState("");
  ...
  const [driveFolderAgate, setDriveFolderAgate] = useState("");
  ...
  const [draftLocalPathAgate, setDraftLocalPathAgate] = useState("");
  ...
  const [draftDriveFolderAgate, setDraftDriveFolderAgate] = useState("");
  ...
  const [editingDriveField, setEditingDriveField] = useState<
    "agate" | "preferences" | "mp3s" | "announcements" | "evergreens" | "shows" | "logs" | null
  >(null);
  ...
  useEffect(() => {
    ...
    fetchNames();
  }, [token, driveFolderAgate, driveFolderPreferences, driveFolderMP3s, driveFolderAnnouncements, driveFolderEvergreens, driveFolderShows, driveFolderLogs]);
  ...
  useEffect(() => {
    if (showLocationsModal) {
      setDraftLocalPathAgate(localPathAgate || "");
      ...
      setDraftDriveFolderAgate(driveFolderAgate || "");
  ...
  useEffect(() => {
    const initSettings = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (serverSettings) {
            ...
            } else if (serverSettings.localPathAgate || serverSettings.localPathMP3s || serverSettings.localPathLogs || serverSettings.localPathCalendar || (serverSettings.mode && serverSettings.mode !== "Demo")) {
      ...
      setLocalPathAgate(settings.localPathAgate || "");
      ...
      setDriveFolderAgate(settings.driveFolderAgate || "");
      ...
      if (isElectron || settings.mode === "Local") {
        const hasValidLocalConfig = !!settings.localPathAgate || (!!settings.localPathMP3s && !!settings.localPathLogs && !!settings.localPathCalendar);
  ...
  const fetchDataForMode = async (settings = getSavedSettings()) => {
    ...
    const hasValidLocalConfig = !!settings.localPathAgate || (!!settings.localPathMP3s && !!settings.localPathCalendar && !!settings.localPathLogs);
    ...
    try {
      if (settings.mode === "Local") {
        try {
          ...
          setInterstitials(normalizeInterstitials(localInterstitials || []));
          setLogs(localLogs || []);
        ...
        const hasPreferencesFolder = !!(DRIVE_FOLDERS.preferences || DRIVE_FOLDERS.agate);
        const hasLogsFolder = !!(DRIVE_FOLDERS.logs || DRIVE_FOLDERS.agate);
        const hasMP3Folder = !!(DRIVE_FOLDERS.announcements || DRIVE_FOLDERS.mp3s || DRIVE_FOLDERS.agate);
    ...
    try {
      ...
      if ((window as any).electronAPI && (window as any).electronAPI.browseFolder) {
        ...
        if (data && data.success && data.path) {
          if (targetField === "agate") setDraftLocalPathAgate(data.path);
    ...
    try {
      ...
      const interstitialsChanged =
        updatedSettings.mode === "Local"
          ? (current.localPathAgate !== updatedSettings.localPathAgate || current.localPathCalendar !== updatedSettings.localPathCalendar)
          : updatedSettings.mode === "Drive"
            ? (current.driveFolderAgate !== updatedSettings.driveFolderAgate || current.driveFolderPreferences !== updatedSettings.driveFolderPreferences)
            : false;
      const logsChanged =
        updatedSettings.mode === "Local"
          ? (current.localPathAgate !== updatedSettings.localPathAgate || current.localPathLogs !== updatedSettings.localPathLogs)
          : updatedSettings.mode === "Drive"
            ? (current.driveFolderAgate !== updatedSettings.driveFolderAgate || current.driveFolderLogs !== updatedSettings.driveFolderLogs)
            : false;
      ...
      if (locationMode === "Local") {
        const announcementsPath = draftLocalPathMediaAnnouncements || draftLocalPathMP3s;
        setLocalPathAgate(draftLocalPathAgate);
      ...
      if (locationMode === "Local") {
        ...
        const hasFolderChanges =
          draftDriveFolderAgate !== driveFolderAgate ||
  ...
  const handleSelectMode = async (mode: "Local" | "Drive" | "Demo") => {
    try {
      ...
      if (mode === "Local") {
        const exists = await checkLocalPathsSafely(
          updatedSettings.localPathAgate || "",
  ...
  return (
    <div className={cn(
      ...
      <LocationsModal
        ...
        draftLocalPathAgate={draftLocalPathAgate}
        setDraftLocalPathAgate={setDraftLocalPathAgate}
        ...
        draftDriveFolderAgate={draftDriveFolderAgate}
        setDraftDriveFolderAgate={setDraftDriveFolderAgate}
            ...
            >
              ...
              <div className="flex flex-col gap-2 w-full pt-1">
                ...
                <button
                  type="button"
                  onClick={() => {
                    if (typeof (window as any).interstitialerDiscardEdits === "function") {
