...
export default function CalendarTab({ interstitials, onSave, isAdmin, onAdminToggle, now, driveMP3s = [], isDriveActive = false, onRefresh, shows = [], onSaveShows, currentViewMode, onViewModeChange, showPixelRuler = false }: CalendarTabProps) {
  ...
  }, [interstitials, shows, now, visibleCalendarDays, calendarDate, driveMP3s]);
  ...
  const getNextIdFromList = (list: Interstitial[]) => {
  ...
  const getNextId = () => {
    return getNextIdFromList(interstitials);
  };

  const fetchLatestStorageInterstitials = async (): Promise<Interstitial[]> => {
    try {
      const settings = getSavedSettings();
      if (settings.mode === 'Drive' && settings.driveFolderPreferences) {
        const list = await loadCalendarFromDrive();
        if (Array.isArray(list) && list.length > 0) {
          return normalizeInterstitials(list);
        }
      } else {
      ...
      console.warn("Could not poll latest storage interstitials before save:", e);
    }
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      let finalNewInterstitials = newInterstitials;
      if (!exists) {
        const latestFromStorage = await fetchLatestStorageInterstitials();
        const freshId = getNextIdFromList(latestFromStorage);
        if (freshId !== updated.id) {
          const rekeyedUpdated: Interstitial = {
            ...updated,
            id: freshId
          };
          finalNewInterstitials = normalizeInterstitials([
      ...
      const result = await onSave(finalNewInterstitials);
      if (result !== false) {
        setEditingId(null);
        if (openedFromAudit) {
          setOpenedFromAudit(false);
