...
const DATA_DIR = IS_PORTABLE ? BASE_DIR : path.join(BASE_DIR, 'data');
const LOG_DIR = IS_PORTABLE ? path.join(BASE_DIR, 'Calendar Logs') : path.join(BASE_DIR, 'Calendar Logs');
...
// For portable instances, interstitial-er_settings.json sits in the same folder as the .exe (no data folder)
// For installed/dev instances, it sits in DATA_DIR.
const SETTINGS_FILE = IS_PORTABLE
...
<body>
  ...
  app.get('/api/stream-local', (req, res) => {
    try {
      ...
      const folderPath = currentSettings.localPathMP3s || DATA_DIR;
      if (!folderPath || !fs.existsSync(folderPath)) {
        return res.status(404).send('Local source folder not defined or offline');
      }

      // Safe basename resolve inside Interstitials subfolder or via search helper
  ...
  // API - Stream local MP3 file by path parameter
  app.get('/api/mp3-file/:fileName', (req, res) => {
    try {
  ...
  app.get('/api/shows', (req, res) => {
    try {
      ...
      if (!fs.existsSync(filePath)) {
        ...
        atomicWriteFileSync(filePath, JSON.stringify({ ShowsBackupCounter: 0, data: defaultShows }, null, 2));
